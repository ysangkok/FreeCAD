...
{
	printf("FeaturePartImportIges::Execute()\n");
...
/*  cout << GetFloatProperty("x") << endl;
  ...
  try{
    ...
    int i=_open(FileName.c_str(),O_RDONLY);
      ...
      printf("FeaturePartImportIges::Execute() not able to open %s!\n",FileName.c_str());
		  return 1;
	  }
    ...
    //  printf( "IGES model contains errors! try loading anyway....\n" );
    ...
    Base::Sequencer().halt();
    printf("FeaturePartImportIges::Execute() failed!");
    return 1;
  }
